...
export interface SocialProfile {
  ...
  bio:string; guildTitle:string; college:string; grade:string; skills:string[]; interests:string[]; attributes:string[]; avatarColor:string;
  avatarConfig?:AvatarConfig|null; avatarUrl?:string|null;
  profileVisibility:'MEMBERS'|'PRIVATE'; presence?:'ONLINE'|'AWAY'|'OFFLINE'; lastSeenAt?:string|null; joinedAt?:string;
}
interface MemberHomepage { profile:SocialProfile; stats:{publishedWorks:number;attendedActivities:number;contributionPoints:number}; works:Array<{id:string;title:string;description:string;createdAt:string}>; activities:Array<{id:string;title:string;startsAt:string;checkedInAt?:string|null}> }
...
export function MemberHomepagePage(){
  ...
  return <main className="social-page member-homepage"><section className="profile-cover" style={{'--profile-accent':profile.avatarColor} as React.CSSProperties}><div className="profile-cover-grid" aria-hidden="true"/><Link className="profile-back" to="/portal/members"><ArrowLeft/>成员名录</Link><div className="profile-identity"><span className="profile-identity-avatar">{profile.avatarUrl?<img className="uploaded-avatar" src={profile.avatarUrl} alt={`${profile.displayName} 的头像`}/>:<PixelAvatar config={profile.avatarConfig} seed={profile.id} size={92} label={`${profile.displayName} 的像素小人`}/>}</span><div><span>UID {profile.uid} · {profile.departmentName??'公会中枢'} · {roleLabel[profile.role]}</span><h1>{profile.displayName}</h1><strong>{profile.guildTitle||'公会成员'}</strong><p>{profile.bio||'这位成员尚未写下自我介绍。'}</p><div className="profile-actions">{user?.id===profile.id?<Link className="guild-button primary" to="/portal/profile"><Edit3/>编辑主页</Link>:<button className="guild-button primary" onClick={()=>chat.mutate()} disabled={chat.isPending}><MessageCircle/>发起私聊</button>}<span className="profile-presence"><i className={`presence ${profile.presence?.toLowerCase()}`}/>{presenceLabel[profile.presence??'OFFLINE']}</span></div></div></div><div className="profile-sigil"><span>ADVENTURER</span><b>{profile.departmentTitle??'GUILD'}</b><small>EST. {profile.joinedAt?new Date(profile.joinedAt).getFullYear():'2018'}</small></div></section>
  <section className="shell profile-body"><div className="profile-main"><div className="profile-stat-grid"><article><Award/><span>贡献记录</span><strong>{data.stats.contributionPoints}</strong></article><article><Sparkles/><span>发布作品</span><strong>{data.stats.publishedWorks}</strong></article><article><CalendarCheck/><span>参与活动</span><strong>{data.stats.attendedActivities}</strong></article></div><article className="profile-section"><div className="section-heading"><Sparkles/><div><small>PORTFOLIO</small><h2>代表作品</h2></div></div>{data.works.length?<div className="profile-work-grid">{data.works.map(work=><div key={work.id}><span>CREATION</span><h3>{work.title}</h3><p>{work.description}</p><time>{formatDate(work.createdAt)}</time></div>)}</div>:<p className="profile-empty">还没有已发布作品。</p>}</article><article className="profile-section"><div className="section-heading"><CalendarCheck/><div><small>ACTIVITY LOG</small><h2>活动足迹</h2></div></div>{data.activities.length?<div className="activity-timeline">{data.activities.map(activity=><div key={activity.id}><i/><span>{formatDate(activity.startsAt)}</span><strong>{activity.title}</strong></div>)}</div>:<p className="profile-empty">活动足迹将在报名或签到后显示。</p>}</article></div><aside className="profile-aside"><article><h2>个人信息</h2><dl><div><dt>学院</dt><dd>{profile.college||'未填写'}</dd></div><div><dt>年级</dt><dd>{profile.grade||'未填写'}</dd></div><div><dt>所属部门</dt><dd>{profile.departmentName??'公会中枢'}</dd></div><div><dt>加入时间</dt><dd>{formatDate(profile.joinedAt)}</dd></div></dl></article><article><h2>冒险属性</h2><div className="social-tags attribute">{profile.attributes?.length?profile.attributes.map(id=><span key={id}>{attributeLabel(id)}</span>):<em>等待补充</em>}</div></article><article><h2>擅长领域</h2><div className="social-tags feature">{profile.skills.length?profile.skills.map(tag=><span key={tag}>{tag}</span>):<em>等待补充</em>}</div></article><article><h2>兴趣坐标</h2><div className="social-tags interest">{profile.interests.length?profile.interests.map(tag=><span key={tag}>{tag}</span>):<em>等待补充</em>}</div></article></aside></section></main>;
}
  ...
  return <main className="social-page">
    ...
    <section className="shell profile-editor-layout">
      <aside className="profile-preview profile-avatar-card" style={{ '--profile-accent': form.avatarColor } as React.CSSProperties}>
        ...
        <h2>{form.displayName || '你的称呼'}</h2><strong>{form.guildTitle || '公会头衔'}</strong><p>{form.bio || '写一段简洁的社团自我介绍。'}</p>
      ...
      <form className="profile-editor" onSubmit={(event: FormEvent) => { event.preventDefault(); setSaved(false); mutation.mutate(); }}>
        ...
        </div><label>个人简介<textarea rows={5} maxLength={500} value={form.bio} onChange={(event) => setForm({ ...form, bio: event.target.value })} /><small>{form.bio.length}/500</small></label></div>
        ...
        <div className="editor-section"><span>04 · APPEARANCE & PRIVACY</span><h2>头像、外观与可见性</h2>
