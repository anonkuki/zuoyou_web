...
export function DepartmentsAdminPage(){
  const {user}=useAuth(); const query=useQuery({queryKey:['departments'],queryFn:()=>api<{items:Department[]}>('/api/public/departments')}); const members=useQuery({queryKey:['admin-members','leaders'],queryFn:()=>api<PageData<Member>>('/api/admin/members?page=1&pageSize=100')}); const refresh=useInvalidate('departments','admin-members','role-hierarchy'); const [editing,setEditing]=useState<Department|null>(null); const update=useMutation({mutationFn:(d:Department)=>api(`/api/admin/departments/${d.id}`,json('PATCH',{title:d.title,description:d.description})),onSuccess:()=>{setEditing(null);refresh();}}); const assign=useMutation({mutationFn:({departmentId,userId}:{departmentId:string;userId:string})=>api(`/api/admin/departments/${departmentId}/leader`,json('POST',{userId})),onSuccess:refresh}); const assignDeputy=useMutation({mutationFn:({departmentId,userId}:{departmentId:string;userId:string})=>api(`/api/admin/roles/${userId}/assign`,json('POST',{role:'DEPARTMENT_ADMIN',departmentId})),onSuccess:refresh});
  return <main><PageHero eyebrow="CLASS ADMINISTRATION" title="部门管理" description="社长可从普通成员中任命部长和副部长；职位变化不影响成员维护自己的个人资料。"/><section className="shell">{query.isLoading?<LoadingPanel/>:<div className="manage-list">{query.data?.items.map(d=>{const departmentMembers=members.data?.items.filter(member=>member.department_id===d.id&&member.is_active)??[];const deputies=departmentMembers.filter(member=>member.role==='DEPARTMENT_ADMIN');return <article key={d.id}><div><small>{d.title}</small><h2>{d.name}</h2><p>{d.description}</p><label>部长<select aria-label={`${d.name}部长`} value={d.leader_id??''} onChange={e=>assign.mutate({departmentId:d.id,userId:e.target.value})}><option value="" disabled>选择部长</option>{departmentMembers.filter(member=>member.role==='MEMBER'||member.id===d.leader_id).map(member=><option value={member.id} key={member.id}>{member.display_name}（UID {member.uid}）</option>)}</select></label><div className="department-deputies"><strong>副部长</strong>{deputies.length?deputies.map(member=><span key={member.id}>{member.display_name}（UID {member.uid}）</span>):<span>暂未任命</span>}</div>{user?.role==='PRESIDENT'&&<label>新增副部长<select aria-label={`${d.name}新增副部长`} value="" onChange={e=>{if(e.target.value)assignDeputy.mutate({departmentId:d.id,userId:e.target.value});}}><option value="">选择普通成员</option>{departmentMembers.filter(member=>member.role==='MEMBER').map(member=><option value={member.id} key={member.id}>{member.display_name}（UID {member.uid}）</option>)}</select></label>}</div><button onClick={()=>setEditing(d)}><UserCog/>编辑部门</button></article>})}</div>}{(assign.error||assignDeputy.error)&&<p className="form-error">{(assign.error||assignDeputy.error)?.message}</p>}{editing&&<form className="inline-create" onSubmit={e=>{e.preventDefault();update.mutate(editing);}}><h2>编辑 {editing.name}</h2><label>职业称号<input value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})}/></label><label>部门说明<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label>{update.error&&<p className="form-error">{update.error.message}</p>}<div className="form-actions"><button type="button" onClick={()=>setEditing(null)}>取消</button><button className="guild-button primary">保存修改</button></div></form>}</section></main>;
}
