...
interface Member { id:string; display_name:string; email:string; role:string; department_id:string|null; is_active:number; created_at:string }
interface Application { id:string; display_name:string; email:string; college:string; department_id:string; reason:string; status:string; rejection_reason?:string; created_at:string }
...
export function MembersAdminPage(){
  const {user}=useAuth(); const [q,setQ]=useState(''); const [editing,setEditing]=useState<Member|null>(null); const refresh=useInvalidate('admin-members','admin-dashboard'); const query=useQuery({queryKey:['admin-members'],queryFn:()=>api<PageData<Member>>('/api/admin/members?page=1&pageSize=100')});
  ...
  const update=useMutation({mutationFn:(member:Member)=>api(`/api/admin/members/${member.id}`,json('PATCH',{displayName:member.display_name,...(user?.role==='ADMIN'&&member.role!=='DEPARTMENT_LEAD'?{role:member.role,departmentId:member.department_id}:{})})),onSuccess:()=>{setEditing(null);refresh();}});
  const items=query.data?.items.filter(m=>`${m.display_name}${m.email}`.toLowerCase().includes(q.toLowerCase()))??[];
  return <AdminListPage title="成员管理" eyebrow="MEMBER ROSTER" description="查询、编辑、停用与恢复成员档案；负责人调整在部门管理中完成。" search={q} onSearch={setQ}>{query.isLoading?<LoadingPanel/>:query.error?<ErrorPanel error={query.error}/>:<><div className="data-table"><div className="table-head"><span>成员</span><span>身份</span><span>部门</span><span>状态</span><span>操作</span></div>{items.map(m=><div className="table-row" key={m.id}><span><strong>{m.display_name}</strong><small>{m.email}</small></span><span>{m.role}</span><span>{m.department_id??'跨部门'}</span><span>{m.is_active?<StatusBadge status="APPROVED"/>:<StatusBadge status="REJECTED"/>}</span><span className="row-actions"><button onClick={()=>setEditing({...m})}><UserCog/>编辑</button>{m.id!==user?.id&&(m.is_active?<button onClick={()=>action.mutate({id:m.id,verb:'deactivate'})}><Ban/>停用</button>:<button onClick={()=>action.mutate({id:m.id,verb:'restore'})}><RotateCcw/>恢复</button>)}</span></div>)}</div>{editing&&<form className="inline-create" onSubmit={e=>{e.preventDefault();update.mutate(editing);}}><h2>编辑成员档案</h2><label>成员称呼<input minLength={2} required value={editing.display_name} onChange={e=>setEditing({...editing,display_name:e.target.value})}/></label>{user?.role==='ADMIN'&&editing.role!=='DEPARTMENT_LEAD'&&<><label>系统角色<select value={editing.role} onChange={e=>setEditing({...editing,role:e.target.value})}><option value="MEMBER">正式成员</option><option value="ADMIN">管理员</option></select></label><label>所属部门<select value={editing.department_id??''} onChange={e=>setEditing({...editing,department_id:e.target.value||null})}><option value="">跨部门</option><option value="dept-cos">COS部</option><option value="dept-tech">技术部</option><option value="dept-music">轻音部</option><option value="dept-original">原创部</option><option value="dept-dance">舞装部</option><option value="dept-publicity">外宣部</option></select></label></>}{editing.role==='DEPARTMENT_LEAD'&&<p className="panel-note">负责人角色和部门请在“部门管理”中调整。</p>}{update.error&&<p className="form-error">{update.error.message}</p>}<div className="form-actions"><button type="button" onClick={()=>setEditing(null)}>取消</button><button className="guild-button primary">保存档案</button></div></form>}</>}</AdminListPage>;
}
...
export function DepartmentsAdminPage(){
  ...
  return <main><PageHero eyebrow="CLASS ADMINISTRATION" title="部门管理" description="维护六个部门的职业称号、职责说明与负责人。"/><section className="shell">{query.isLoading?<LoadingPanel/>:<div className="manage-list">{query.data?.items.map(d=><article key={d.id}><div><small>{d.title}</small><h2>{d.name}</h2><p>{d.description}</p><label>部门负责人<select aria-label={`${d.name}负责人`} value={d.leader_id??''} onChange={e=>assign.mutate({departmentId:d.id,userId:e.target.value})}><option value="" disabled>选择负责人</option>{members.data?.items.filter(member=>member.department_id===d.id&&member.is_active).map(member=><option value={member.id} key={member.id}>{member.display_name}</option>)}</select></label></div><button onClick={()=>setEditing(d)}><UserCog/>编辑部门</button></article>)}</div>}{editing&&<form className="inline-create" onSubmit={e=>{e.preventDefault();update.mutate(editing);}}><h2>编辑 {editing.name}</h2><label>职业称号<input value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})}/></label><label>部门说明<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label>{update.error&&<p className="form-error">{update.error.message}</p>}<div className="form-actions"><button type="button" onClick={()=>setEditing(null)}>取消</button><button className="guild-button primary">保存修改</button></div></form>}</section></main>;
}
...
export function RecruitmentAdminPage(){
  const refresh=useInvalidate('admin-applications','admin-dashboard'); const query=useQuery({queryKey:['admin-applications'],queryFn:()=>api<PageData<Application>>('/api/admin/applications?page=1&pageSize=100')}); const action=useMutation({mutationFn:({id,verb,body}:{id:string;verb:string;body?:unknown})=>api(`/api/admin/applications/${id}/${verb}`,json('POST',body)),onSuccess:data=>{const result=data as {activationCode?:string};if(result.activationCode)navigator.clipboard?.writeText(result.activationCode);refresh();}});
  return <main><PageHero eyebrow="RECRUITMENT DESK" title="招新管理" description="审核申请；通过后系统创建未激活成员并生成一次性激活码。"/><section className="shell">{query.isLoading?<LoadingPanel/>:query.error?<ErrorPanel error={query.error}/>:<div className="manage-list">{query.data?.items.map(a=><article key={a.id}><div><StatusBadge status={a.status}/><h2>{a.display_name}</h2><p>{a.college} · {a.email}</p><p>{a.reason}</p><small>{formatDate(a.created_at)}</small></div>{a.status==='PENDING'&&<div className="row-actions"><button onClick={()=>action.mutate({id:a.id,verb:'approve'})}><Check/>通过并复制激活码</button><button className="danger" onClick={()=>{const reason=prompt('填写拒绝原因');if(reason)action.mutate({id:a.id,verb:'reject',body:{reason}})}}><X/>拒绝</button></div>}{a.status==='APPROVED'&&<button onClick={()=>action.mutate({id:a.id,verb:'regenerate-activation'})}>重新生成激活码</button>}</article>)}</div>}</section></main>;
}
  ...
  return <main>
    ...
    <section className="shell">
      {open&&<form className="inline-create" onSubmit={e=>{e.preventDefault();upload.mutate();}}>
        ...
        <label>可见范围<select value={visibility} onChange={e=>setVisibility(e.target.value)}><option value="MEMBERS">全体成员</option><option value="DEPARTMENT">仅所属部门</option>{user?.role==='ADMIN'&&<><option value="PUBLIC">公开</option><option value="ADMINS">仅管理员</option></>}</select></label>
        ...
        <label>所属部门<select value={departmentId} disabled={user?.role!=='ADMIN'} onChange={e=>setDepartmentId(e.target.value)}><option value="dept-cos">COS部</option><option value="dept-tech">技术部</option><option value="dept-music">轻音部</option><option value="dept-original">原创部</option><option value="dept-dance">舞装部</option><option value="dept-publicity">外宣部</option></select></label>
...
function AdminListPage({title,eyebrow,description,search,onSearch,children}:{title:string;eyebrow:string;description:string;search:string;onSearch:(value:string)=>void;children:React.ReactNode}){return <main><PageHero eyebrow={eyebrow} title={title} description={description}/><section className="shell"><label className="search-box"><Search/><span className="sr-only">搜索</span><input value={search} onChange={e=>onSearch(e.target.value)} placeholder="搜索成员名称或邮箱"/></label>{children}</section></main>}
