...
export function PixelFooter() {
  return <footer className="pixel-footer">
    <div className="pixel-footer-main">
      ...
      <section className="online-party"><h2>在线冒险者 <span>● 128人在线</span></h2><div>{members.map(([name,color],index)=><span className={`footer-avatar avatar-${index+1}`} data-facing="visitor" style={{'--avatar-color': color} as React.CSSProperties} title={name} key={name}><i/><b>{name.slice(0,1)}</b><em/></span>)}</div></section>
    </div>
