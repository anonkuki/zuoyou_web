...
interface Member { id:string; display_name:string; email:string; role:Role; department_id:string|null; is_active:number; created_at:string }
interface HierarchyAssignment { id:string; userId:string; role:Role; departmentId:string|null; displayName:string; departmentName:string|null; grantedByName:string|null; grantedAt:string }
...
export function MembersAdminPage(){
  const {user}=useAuth(); const [q,setQ]=useState(''); const [editing,setEditing]=useState<Member|null>(null); const refresh=useInvalidate('admin-members','admin-dashboard','role-hierarchy','departments'); const query=useQuery({queryKey:['admin-members'],queryFn:()=>api<PageData<Member>>('/api/admin/members?page=1&pageSize=100')});
