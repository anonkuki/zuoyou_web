...
export function LoginPage() {
  ...
  return <main className="auth-page"><section className="auth-card auth-choice-card">
    ...
    {mode === 'login' ? <form onSubmit={(event: FormEvent) => { event.preventDefault(); login.mutate(); }}>
      ...
      <label>密码<input autoComplete="current-password" type="password" required minLength={6} value={loginForm.password} onChange={event => setLoginForm({ ...loginForm, password: event.target.value })} /></label>
