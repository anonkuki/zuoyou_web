...
import { Award, CalendarCheck, CheckCircle2, ClipboardCheck, Download, FileArchive, Gamepad2, Megaphone, Palette, Sparkles, UserRound } from 'lucide-react';
...
export function PortalHomePage(){
  ...
  return <main><PageHero eyebrow="MEMBER LODGE" title={`欢迎回来，${user?.displayName ?? '冒险者'}`} description="这里记录你在真实社团协作中的活动、创作与任务。"/><section className="shell portal-identity-card"><div className="portal-identity-avatar"><PixelAvatar config={user?.avatarConfig} seed={user?.id ?? 'guest'} size={96} label="我的像素小人"/></div><div className="portal-identity-copy"><small>MY PIXEL SELF</small><h2>{user?.displayName ?? '冒险者'}</h2><p>{user?.guildTitle || '公会成员'} · 这是你在广场与名录里的像素形象</p><div className="portal-identity-actions"><Link className="guild-button primary" to="/portal/avatar"><Palette/>捏脸</Link><Link to="/portal/profile"><UserRound/>编辑资料</Link><Link to="/portal/world"><Gamepad2/>像素广场</Link></div></div></section><section className="shell dashboard-cards"><article><UserRound/><span>当前身份</span><strong>{user?roleLabels[user.role]:'正式成员'}</strong></article><article><ClipboardCheck/><span>待办任务</span><strong>{tasks.data?.items.filter(t=>!t.confirmed_at).length??0}</strong></article><article><Sparkles/><span>创作入口</span><strong>发帖</strong></article><article><Award/><span>公会贡献</span><strong>{contribution.data?.points??0}</strong></article></section><section className="shell"><article className="parchment-panel portal-announcements"><h2><Megaphone/>最新公告</h2>{announcements.data?.items.length?<div className="quest-list">{announcements.data.items.map(item=><Link className="quest-row" key={item.id} to={`/announcements/${item.id}`}><Megaphone/><span>{item.title}</span><strong>{formatDate(item.publishedAt)}</strong></Link>)}</div>:<p>暂无公告，去大厅告示板看看历史公告。</p>}</article></section><section className="shell detail-columns"><article className="parchment-panel"><h2>快捷行动</h2><div className="quick-actions"><Link to="/portal/activities"><CalendarCheck/>活动报名与签到</Link><Link to="/tavern?compose=1"><Megaphone/>发布帖子或上传附件</Link><Link to="/portal/tasks"><ClipboardCheck/>处理部门任务</Link><Link to="/portal/files"><FileArchive/>查看内部资料</Link></div></article><article className="dark-panel"><h2>近期贡献记录</h2>{contribution.data?.events.length?contribution.data.events.slice(0,4).map(event=><div className="quest-row" key={`${event.action}-${event.entityId}`}><Award/><span>{contributionLabel[event.action]??event.action}</span><strong>+{event.points}</strong></div>):<p>完成签到、作品发布或任务确认后将在这里留下记录。</p>}<p>贡献值是社团协作统计，不是游戏货币。</p></article></section></main>;
}
