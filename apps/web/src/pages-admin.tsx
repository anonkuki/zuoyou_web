...
export function MembersAdminPage(){
  ...
  const canRevoke=(member:Member)=>user?.role==='PRESIDENT'&&(member.role==='VICE_PRESIDENT'||member.role==='DEPARTMENT_HEAD'||member.role==='DEPARTMENT_ADMIN')
...
export function DepartmentsAdminPage(){
  ...
  const roleMembers=useQuery({queryKey:['department-role-members'],queryFn:()=>api<{items:DepartmentRoleMember[]}>('/api/admin/department-role-members')});
  const refresh=useInvalidate('departments','department-role-members','admin-members','role-hierarchy');
  const [editing,setEditing]=useState<Department|null>(null);
  const update=useMutation({mutationFn:(d:Department)=>api(`/api/admin/departments/${d.id}`,json('PATCH',{title:d.title,description:d.description})),onSuccess:()=>{setEditing(null);refresh();}});
  const assign=useMutation({mutationFn:({departmentId,userId}:{departmentId:string;userId:string})=>api(`/api/admin/departments/${departmentId}/leader`,json('POST',{userId})),onSuccess:refresh});
  const assignDeputy=useMutation({mutationFn:({departmentId,userId}:{departmentId:string;userId:string})=>api(`/api/admin/roles/${userId}/assign`,json('POST',{role:'DEPARTMENT_ADMIN',departmentId})),onSuccess:refresh});
  const revoke=useMutation({mutationFn:(userId:string)=>api(`/api/admin/roles/${userId}/revoke`,json('POST')),onSuccess:refresh});
  const actionError=assign.error||assignDeputy.error||revoke.error;

  return <main><PageHero eyebrow="CLASS ADMINISTRATION" title="部门管理" description="社长可从该部门全部正式成员中任命或撤销部长和副部长；新通过审核的成员会直接出现在候选列表顶部。"/><section className="shell">{query.isLoading||roleMembers.isLoading?<LoadingPanel/>:query.error?<ErrorPanel error={query.error}/>:roleMembers.error?<ErrorPanel error={roleMembers.error}/>:<div className="manage-list">{query.data?.items.map(d=>{
    const departmentMembers=roleMembers.data?.items.filter(member=>member.departmentIds.includes(d.id))??[];
    const leader=departmentMembers.find(member=>member.id===d.leader_id||(member.role==='DEPARTMENT_HEAD'&&member.department_id===d.id));
    const deputies=departmentMembers.filter(member=>member.role==='DEPARTMENT_ADMIN'&&member.department_id===d.id);
    const candidates=departmentMembers.filter(member=>member.role==='MEMBER');
    return <article key={d.id}><div><small>{d.title}</small><h2>{d.name}</h2><p>{d.description}</p><div className="department-role-row"><label>部长<select aria-label={`${d.name}部长`} value={leader?.id??''} onChange={e=>assign.mutate({departmentId:d.id,userId:e.target.value})}><option value="" disabled>选择部长</option>{leader&&<option value={leader.id}>{leader.display_name}（UID {leader.uid}）</option>}{candidates.map(member=><option value={member.id} key={member.id}>{member.display_name}（UID {member.uid}）</option>)}</select></label>{leader&&user&&isExecutiveRole(user.role)&&user.id!==leader.id&&<button className="danger" aria-label={`撤销 ${d.name} 部长${leader.display_name}`} disabled={revoke.isPending} onClick={()=>revoke.mutate(leader.id)}><X/>撤销部长</button>}</div><div className="department-deputies"><strong>副部长</strong>{deputies.length?deputies.map(member=><span className="department-deputy" key={member.id}>{member.display_name}（UID {member.uid}）{user?.role==='PRESIDENT'&&user.id!==member.id&&<button className="danger" aria-label={`撤销 ${d.name} 副部长${member.display_name}`} disabled={revoke.isPending} onClick={()=>revoke.mutate(member.id)}><X/>撤销</button>}</span>):<span>暂未任命</span>}</div>{user?.role==='PRESIDENT'&&<label>新增副部长<select aria-label={`${d.name}新增副部长`} value="" onChange={e=>{if(e.target.value)assignDeputy.mutate({departmentId:d.id,userId:e.target.value});}}><option value="">选择普通成员</option>{candidates.map(member=><option value={member.id} key={member.id}>{member.display_name}（UID {member.uid}）</option>)}</select></label>}<small className="department-role-note">候选仅包含已通过审核并加入本部门的有效账号，共 {candidates.length} 人；最新账号优先显示。</small></div><button onClick={()=>setEditing(d)}><UserCog/>编辑部门</button></article>;
  })}</div>}{actionError&&<p className="form-error">{actionError.message}</p>}{editing&&<form className="inline-create" onSubmit={e=>{e.preventDefault();update.mutate(editing);}}><h2>编辑 {editing.name}</h2><label>职业称号<input value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})}/></label><label>部门说明<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label>{update.error&&<p className="form-error">{update.error.message}</p>}<div className="form-actions"><button type="button" onClick={()=>setEditing(null)}>取消</button><button className="guild-button primary">保存修改</button></div></form>}</section></main>;
}
...
export function SettingsAdminPage(){
  ...
  return <main><PageHero eyebrow="GUILD CONFIGURATION" title="系统设置" description="维护站点名称、首页公会状态和招新开关。"/><section className="shell narrow">{query.isLoading?<LoadingPanel/>:<form className="parchment-form" onSubmit={e=>{e.preventDefault();save.mutate();}}>
    ...
    <label>成立年份<input type="number" min="1900" max="2200" value={settings.foundedYear??'1999'} onChange={e=>setValue('foundedYear',e.target.value)}/></label>
